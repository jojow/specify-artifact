...
module.exports = (input, options, done) => {
  ...
  async.series([
    ...
    cleanupPaths = _.uniq(cleanupPaths);
      ...
      cleanup: (exclude, done) => {
        if (!_.isFunction(done) && _.isFunction(exclude)) {
          done = exclude;
          exclude = null;
        ...
        async.eachSeries(cleanupPaths, function(p, done) {
          if (_.reduce(exclude, (result, e) => {
            const doRemove = !_.startsWith(p, e) && !_.startsWith(e, p);
            return doRemove && result;
          }, true)) {
            fs.remove(p, done);
          } else {
            done();
