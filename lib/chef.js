  ...
  async.series([
    function(done) {
      prepare(input, options, (err, output) => {
        ...
        if (result.type === 'file') dir = path.dirname(result.path);
        else dir = result.path;
...
const resolve = (args, done) => {
  args = args || {};

  if (!args.path || args.type !== 'dir') return done(new Error('cookbook path missing or not a directory'));

  const metadata = JSON.parse(fs.readFileSync(path.resolve(args.path, 'metadata.json')));
  const depsDir = path.resolve(args.path, 'cookbook_dependencies');
